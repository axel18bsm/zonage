...
var
  ...
  ZoneCount    : Integer;
  PixX         : array[0..MAX_PTS-1]   of SmallInt;
  PixY         : array[0..MAX_PTS-1]   of SmallInt;
  TotalPts     : Integer;
  { affichage }
  MapTex       : TTexture2D;
  HoveredIdx   : Integer;
  ...
  DragOffY     : Single;
  WinW, WinH   : Integer;
  { tampon DrawTriangleFan/DrawLineStrip (global = pas de stack overflow) }
  pts          : array[0..MAX_PTS+1] of TVector2;
  { boucle principale }
  Zi           : Integer;
  Pi2          : Integer;
  Cx2, Cy2     : Single;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          PixX[TotalPts] := SmallInt(Round(gx * scaleX));
          PixY[TotalPts] := SmallInt(Round(-gy * scaleY));
          Inc(TotalPts);
          Inc(ptCount);
        end;
      end
...
begin
  ...
  begin
    ...
    for j := ZoneStart[i] to ZoneStart[i] + ZonePtCount[i] - 1 do
      line := line + ';' + IntToStr(PixX[j]) + ';' + IntToStr(PixY[j]);
    WriteLn(f, line);
  end;
  CloseFile(f);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        PixX[TotalPts] := StrToInt(parts[p]);
        PixY[TotalPts] := StrToInt(parts[p + 1]);
        Inc(TotalPts);
        Inc(p, 2);
      end;
    end;
...
{ ============================================================
  ...
  ============================================================ }
function PointInPolygon(px, py   : Single;
                        startIdx : Integer;
...
begin
  ...
  begin
    if PointInPolygon(px, py, ZoneStart[i], ZonePtCount[i]) then
      Result := i;
    Dec(i);
  end;
end;
...
procedure DrawZonePoly(zoneIdx   : Integer;
                       ...
                       fillCol   : TColorB;
                       borderCol : TColorB);
{ DrawTriangleFan  : premier point = centre, puis les sommets dans l ordre
  DrawLineStrip    : contour ferme (dernier point = premier point)
  Les deux attendent un PVector2 -> tableau local TVector2 }
var
  i        : Integer;
  n        : Integer;
  cx, cy   : Double;
  scx, scy : Single;
  sx, sy   : Single;
  { pts est declare en variable globale pour eviter stack overflow }
begin
  ...
  pts[0] := Vector2Create(scx, scy);

  { [1..n] = sommets dans l ordre }
  for i := 0 to n - 1 do
  begin
    MapToScreen(PixX[startIdx + i], PixY[startIdx + i], sx, sy);
    pts[i + 1] := Vector2Create(sx, sy);
  end;

  { [n+1] = fermeture du fan = repeter le premier sommet }
  pts[n + 1] := pts[1];

  { remplissage : centre + n sommets + fermeture = n+2 points }
  DrawTriangleFan(@pts[0], n + 2, fillCol);

  { contour : DrawLineStrip sur les n sommets + fermeture = n+1 points }
  DrawLineStrip(@pts[1], n + 1, borderCol);
end;
